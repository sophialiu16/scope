...
\begin{fullfigure} 
	...
	\caption{Image of display while oscilloscope is on, before triggering.} 
	\label{Display}    
\end{fullfigure}
...
\begin{description}
	...
	\index{Menu!Level}\item[Level] The Level menu entry sets the trigger level. It can be set to any value from the most negative input voltage (0) to the most positive (3.8 V) in 128 steps. Additionally, the trigger level is displayed as a line on the screen when the trigger level is being changed.
...
% MEMORY MAP
The oscilloscope system is centered around a Cyclone III FPGA, which includes a Nios II CPU and controllers for the VRAM, LCD, rotary encoders, and analog input and triggering. Upon startup, the configuration data is loaded from the serial EEPROM. The program is then loaded from the ROM, with the program data stored in the RAM. 

\marginlabel{Receiving a Signal:} When the probe is connected to a desired input, the analog signal is sent to an analog to digital converter chip. Data is sent to the FPGA to an analog controller. The various trigger setting inputs (auto triggering, trigger enable, trigger slope, trigger level) are used along with the 8 bit signal to determine if the system should begin sampling. Once triggering has occurred, the signal is written to a %SIZE, 512 
...
\marginlabel{User Input:} Two rotary encoders are used for navigating through the menu. A rotary controller in the FPGA decodes and debounces the input.

% the other things 

\subsubsection{FPGA} \index{FPGA}
\begin{fullfigure} 
	\centering
...
\includepdf[pages={2}, fitpaper=true]{data/Schematics.pdf} 

An Altera Cyclone III FPGA was used for all the logic and processing required (U9, EP3C25Q240). The three required voltage levels were generated with linear regulators, and each power pin was locally bypassed. The full schematics with the FPGA can be seen in Appendix~\ref{AP:Schematics}.
...
\begin{fullfigure} 
	\centering
	\includegraphics[width=1\textwidth]{{data/Board_Layout_Power.png}}
	\caption{Board Layout: Power circuitry.} 
	\label{Layout:Power}    
\end{fullfigure}

\includepdf[pages={4}, fitpaper=true]{data/Schematics.pdf}
...
The ROM timing diagram for the read cycle can be seen at Appendix~\ref{AP:Timing}, Figure~\ref{AP:ROM_Read_Timing}. 3 wait states were used for the read cycle. %is this the right way to say it?
...
A serial configuration device (U4, EPCS16), a 2 MB flash memory device, was used to store FPGA configuration data and was programmed with the Altera Quartus II toolset \footnote{Was unable to successfully use serial device for unknown reasons, possibly due to incorrect FPGA configurations.}.
...
\subsection{Changes} 
The incorrect foootprint was used for the ADC in stock (SOP24, not TSSOP24), so all the ADC connections were rewired using the prototype area. The reset input was also connected to an unusable configuration pin and was rewired to an unused input pin. The LCD connector footprint was also numbered inversely to the schematic block, so the connector had to be soldered backwards. 
